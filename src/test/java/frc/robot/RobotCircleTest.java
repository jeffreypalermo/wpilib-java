...
class RobotCircleTest {
  ...
  void innerSlowerThanOuter() {
    double[] lr = Robot.computeCircleTankOutputs(10.0, 2.0, 0.5);
  ...
  void outputsClamped() {
    double[] lr = Robot.computeCircleTankOutputs(10.0, 100.0, 1.0);
  ...
  void tinyRadiusFinite() {
    double[] lr = Robot.computeCircleTankOutputs(0.0001, 2.0, 0.4);
  ...
  void guardRails() {
    double[] lr = Robot.computeCircleTankOutputs(-5.0, -1.0, -0.2);
    assertTrue(lr[0] >= -1 && lr[0] <= 1);
    assertTrue(lr[1] >= -1 && lr[1] <= 1);
  }
}
